...
def rest(workers,spshifts):
  ...
  for worker in workers:
    if datetime.strptime(worker.endtime, '%H:%M')-datetime.strptime(worker.starttime, '%H:%M') > timedelta(hours=restnd_mint) :
      worker.append(((datetime.strptime(worker.starttime, '%H:%M').hour+datetime.strptime(worker.endtime, '%H:%M').hour)+((datetime.strptime(worker.starttime, '%H:%M').minute+datetime.strptime(worker.endtime, '%H:%M').minute)/60))/2)
